package com.hicomb.cloud.rest.util;
...
public class HttpsUtil {
	...
	public static SSLSocketFactory getSSLSocketFactory(String serverName,int port,String certificateStorePassword ) throws RestException{
		try {
			if(cachedSocketFactory.get(serverName) == null){
		        ...
		        TrustManagerFactory tmf = TrustManagerFactory.getInstance(CommonConst.SYSDIRECTOR_SSL_ALGORITHM);
		        tmf.init(ts);
	...
	private static KeyStore createKeyStore(String host, int port, File keystoreFile, char [] password) throws KeyStoreException, NoSuchAlgorithmException, CertificateException, IOException, KeyManagementException  {
		...
		Certificate firstCert = getDirectorCertificate(host, port);
	 ...
	 */
	private static Certificate getDirectorCertificate(String host, int port) throws NoSuchAlgorithmException, KeyManagementException, IOException  {
		URL url = new URL("https://" + host + ":" + Integer.toString(port) + "/ibm/director/rest");
